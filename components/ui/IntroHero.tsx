...
const BLOB_LAYOUT = [
  { position: { top: '-12%', left: '-8%' }, size: '46vmax', orbit: 'intro-orbit-a', dur: '53s', delay: '0s', bdur: '19s', bdelay: '0s' },
  { position: { top: '-18%', right: '-10%' }, size: '42vmax', orbit: 'intro-orbit-b', dur: '61s', delay: '-13s', bdur: '23s', bdelay: '-5s' },
  { position: { bottom: '-16%', left: '4%' }, size: '44vmax', orbit: 'intro-orbit-c', dur: '47s', delay: '-7s', bdur: '29s', bdelay: '-11s' },
  { position: { bottom: '-20%', right: '-6%' }, size: '40vmax', orbit: 'intro-orbit-a', dur: '67s', delay: '-23s', bdur: '17s', bdelay: '-3s' },
  { position: { top: '26%', left: '-14%' }, size: '34vmax', orbit: 'intro-orbit-b', dur: '43s', delay: '-31s', bdur: '31s', bdelay: '-9s' },
] as const;
...
export function IntroHero() {
  ...
  // useFocusTrap only constrains Tab; it doesn't move initial focus, so do
  // that here or keyboard users can't reach the dismiss control.
  useEffect(() => {
    if (phase === 'visible') containerRef.current?.querySelector('button')?.focus();
  }, [phase]);
  ...
  return (
    <div
      ...
      className={`fixed inset-0 z-[60] flex flex-col items-center overflow-hidden bg-panel-bg px-6 text-center transition-[opacity,transform] duration-700 ease-out motion-reduce:transition-none ${
    ...
    >
      ...
      <div className="relative z-10 flex flex-1 flex-col items-center justify-center">
        ...
        <p className="mt-6 max-w-xl text-balance text-base text-text-secondary sm:text-lg">
          Welcome to Seattle Atlas, an interactive map of Seattle&rsquo;s zoning, transit, and
          bike infrastructure.
        </p>
      </div>
      ...
      <button
        type="button"
        onClick={dismiss}
        className="relative z-10 flex flex-col items-center gap-2 pt-10 pb-12 text-sm text-text-secondary transition-colors hover:text-text-primary focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-accent sm:pb-16"
      >
