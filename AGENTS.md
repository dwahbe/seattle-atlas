...
- Each page shows a Mapbox Static Images preview (`staticMapUrl()` in `lib/static-map.ts`) rendered from a published Studio style (`STATIC_MAP_STYLE`: satellite imagery + zoning fills). The style JSON is generated by `bun scripts/generate-static-map-style.ts` and pushed to the existing Studio styles in place by `bun scripts/upload-static-map-style.ts` (Styles API PATCH, so the style ids never change; needs `MAPBOX_UPLOAD_TOKEN` with `styles:write`, and the script checks the published version and says if a Studio publish is still needed) — if the zoning legend changes, re-run both and refresh the social-card render (`bun scripts/generate-og-map.ts`).
...
- **Fills sit beneath the base water layer:** `MapLayers.findBeforeId` inserts the site's fill layers (zoning, parks, institutions) before the base style's `water` fill (`BASE_WATER_LAYER_ID`, `lib/constants.ts`) when no higher-zOrder site fill is on the map, while lines and circles still go on top. Water, piers, bridges, roads, and labels therefore paint above the zoning tint, and zoning/park color never shows over water — the city's zoning covers the platted tidelands off Magnolia and the parks source has lots under Puget Sound; both still render, hidden under the water. `isOverWater` (`lib/mapbox.ts`) gates `queryInspectableFeature`, and the hover query includes the water layer and treats a topmost water hit as empty, so water is never inspectable (which also means zoned areas the basemap draws as water, e.g. Shilshole Bay Marina, can't be inspected). `generate-static-map-style.ts` applies the same stacking to the static previews (satellite has no water fill, so it adds a flat `seattle-atlas-water` layer above the fills, below the first road layer). Separately, `prepare-parks-data.ts` drops the two all-water tideland "parks" (Magnolia Tidelands Park, Discovery Park Tidelands) so the land-area stat stays honest.
