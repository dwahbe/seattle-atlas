...
- **Tailwind CSS v4** — theme tokens defined in `globals.css` via `@theme`. Use semantic classes (`text-text-primary`, `bg-panel-bg`, `border-border`), not raw `rgb(var(...))`. **Touch-target gotcha:** a global `@media (hover: none) and (pointer: coarse)` rule in `globals.css` forces `min-height/min-width: 44px` on every `button`, `a`, and `[role="button"]`. This applies on touch devices only, so it silently inflates layout on mobile but not desktop. Fine-print / inline links (e.g. attributions) must add the `.touch-target-inline` escape-hatch class (also defined in that block) to opt out. **Flex-item links top-align on touch:** a link (or a `flex` button) that is itself a flex/grid item gets blockified, so the 44px `min-height` applies and its text sits at the _top_ of the box while `items-center` siblings (icons, non-link text) stay in the middle — invisible on desktop, obvious on iPad/iPhone. Give such links `inline-flex items-center` so the text centers inside the tap box (see `Breadcrumb`, the `Footer` nav); short labels (< 44px) also hit the `min-width`, so pad them past it (`px-2`, moving the spacing off the parent's `gap`) or add `justify-center`. Inline links inside prose are unaffected — inline boxes ignore `min-height`/`min-width`.
...
- `pin` — inspect marker, i.e. the original click/search point (`lat,lng` in the URL, `[lng, lat]` internally via `parsePinParam`/`serializePinParam`, which wraps longitude into ±180 because Mapbox reports unwrapped lngs across world copies). The inspected feature object is never serialized — on load, `MapGL`'s one-shot deep-link restore queries at the pin after the map's first `idle` to rebuild the panel, marker, and highlight. `inspect` alone can't reopen the panel (vector-tile features aren't fetchable by ID) but is passed as `expectedFeatureId` to break ties between stacked polygons; the pin doubles as the anchor for parcel/permit lookups, so it must stay the exact original point.

  **The restore is not a synthetic click.** It reads its target from the URL once at mount (never from the live pin, so a click or search can't trigger it), leaves the URL untouched on a query miss (a click on empty space clears it — that would destroy the shared link), and recenters on an off-viewport pin rather than giving up. It reports through `onFeatureClick(..., { restored: true })`; consumers use that flag to stay out of the way of a panel nobody asked for (`InspectPanel` skips its focus grab, the neighborhood highlight survives). `OnboardingTour` sits out any load with a `pin` (`hasPinParam`).
...
- Generated: `apple-icon.tsx`, `icon.svg`, `sitemap.ts`, `robots.ts`; plus `error.tsx`, `not-found.tsx`, `loading.tsx`. The social card is the static `app/opengraph-image.png` (+ `opengraph-image.alt.txt`) — the brand mark and wordmark over the intro splash's aurora (soft radial blobs of the simplified zoning palette via `getLegendCategories`, wordmark in Instrument Sans Bold fetched at generation time), drawn by `bun scripts/generate-og-map.ts`. `OG_IMAGE` (`lib/og-image.ts`) is the single source for the card's URL/type/size/alt — the script derives its render size from it and writes the alt file. Every page references `OG_IMAGE` in its `openGraph.images` — required because a page-level `openGraph` export replaces the inherited object and silently drops the root file-convention image — and the home/seattle-zoning JSON-LD image URLs build from `OG_IMAGE.url`. There is deliberately no twitter-image; X falls back to og:image. Re-run the script (and commit the PNG) after brand-mark or legend-palette changes.
...
- Each page shows a Mapbox Static Images preview (`staticMapUrl()` in `lib/static-map.ts`) rendered from a published Studio style (`STATIC_MAP_STYLE`: satellite imagery + zoning fills). The style JSON is generated by `bun scripts/generate-static-map-style.ts` and pushed to the existing Studio style in place by `bun scripts/upload-static-map-style.ts` (Styles API PATCH to both the published style and its Studio draft, so the id never changes and a later Studio "Publish" can't revert the live style; needs `MAPBOX_UPLOAD_TOKEN` with `styles:write`, and the script verifies both versions afterwards) — if the zoning legend changes, re-run both and refresh the social-card render (`bun scripts/generate-og-map.ts`).
...
- **One query-at-point implementation:** `queryInspectableFeature(map, lngLat, activeLayers, opts)` (`lib/mapbox.ts`) builds the `InspectedFeature` — excluding `NON_INSPECTABLE_LAYER_IDS`, attaching institution enrichment, and resolving stacked polygons via `preferZoning` (address search) / `expectedFeatureId` (deep-link restore). Live clicks, search, and restore all call it; don't add a parallel copy.
- The `institutions` layer is a silent enrichment source (fill-opacity 0, empty legend, no controls toggle). `queryInspectableFeature` runs a secondary `queryRenderedFeatures` against it and attaches matched institution data to the inspected feature, which `ZoningSummary` then uses to swap the building graphic for the institution's logo + name. Canonical data + logo lookup live in `lib/institutions.ts` and `components/ui/InstitutionGraphic.tsx`. Logos are bundled in `public/institutions/{lowercase-code}.{ext}` keyed on the tileset's `OVERLAY` code (e.g. `mio-uw.png`); codes without a logo fall back to a category icon (university/college/hospital). Swedish's three campuses share one logo file.
- **Fills sit beneath the base water layer, which is lifted above the roads:** on each style load `MapLayers` runs `restackBaseWater` (`lib/mapbox.ts`), moving the base style's `water` fill and the two `land-structure` layers to just below the first `bridge-` layer (`BASE_BRIDGE_ANCHOR_LAYER_ID` / `BASE_LIFTED_LAYER_IDS`, `lib/constants.ts`). `findBeforeId` then inserts the site's fill layers (zoning, parks, institutions) before `water` when no higher-zOrder site fill is on the map, while lines and circles still go on top. Result: buildings, tunnels, and surface roads stay tinted under the fills as they always were, water covers the fills, and bridges and labels paint over the water. Because the base labels now draw above the tint, where Mapbox's pale gray text and white halo read as a glow and lose contrast, `lib/map-labels.ts` holds the site's label design: `LABEL_CLASSES` maps each base label class (place, neighborhood, road, water, poi, airport, region) to an ink from `MAP_LABEL_COLORS` (`lib/constants.ts`, mirrors the `--text-*` tokens plus an accent-hue water blue; streets and POIs use the primary ink in dark mode, where the gray loses against the light fills), a halo width (0 = crisp), a DIN Pro weight (Bold for places and neighborhoods, Medium for streets and POIs), and a zoom range (neighborhoods linger to z16, streets and POIs wait until z13). `applyLabelClasses` applies it while a base layer is active and restores the base values when it's off; `MapLayers` takes `isDark` for this and the memo resets on every style reload. Tune labels in that table, not in MapLayers — so zoning/park color never shows over water (the city's zoning covers the platted tidelands off Magnolia and the parks source has lots under Puget Sound; both still render, hidden). `isOverWater` gates `queryInspectableFeature`, and the hover query includes the water layer and treats a topmost water hit as empty, so water is never inspectable (which also means zoned areas the basemap draws as water, e.g. Shilshole Bay Marina, can't be inspected). `generate-static-map-style.ts` applies the same stacking to the static previews (satellite has no water fill, so it adds a flat `seattle-atlas-water` layer above the fills). Separately, `prepare-parks-data.ts` drops the two all-water tideland "parks" (Magnolia Tidelands Park, Discovery Park Tidelands) so the land-area stat stays honest.
...
- `MAPBOX_UPLOAD_TOKEN` — optional secret token (`uploads:read` + `uploads:write` + `styles:write`), used only by `scripts/upload-parks-tileset.ts` (replace the parks tileset in place after `bun scripts/prepare-parks-data.ts`) and `scripts/upload-static-map-style.ts` (push the static map styles). Not needed to run the site.
